...
export const TokenAnalysis: React.FC<{ target?: string; onBack: () => void }> = ({ target, onBack }) => {
      ...
      try {
        ...
        while (!completed && attempts < 10) {
          ...
          if (status.status !== 'PENDING') {
            ...
            // Fetch all parts
            const [metaData, riskData, holderData, liqData, tempData] = await Promise.all([
  ...
  const integrityIndex = risk?.compositeRiskScore || 0;
  const aiVerdict = summary?.verdict || '';
  const aiConfidence = 95 + (integrityIndex % 5);

  const ownershipData = holders ? [
    { name: 'Top 10 Wallets', value: holders.top10Concentration, color: '#137fec' },
    { name: 'Creator', value: holders.creatorShare, color: '#10b981' },
    { name: 'Others', value: 100 - holders.top10Concentration - holders.creatorShare, color: '#64748b' },
  ] : [];
  ...
  const activityData = temporal ? [
    { day: 'Launch', volume: temporal.phases?.find(p => p.phase === 'launch')?.score || 0 },
    { day: 'Accumulation', volume: temporal.phases?.find(p => p.phase === 'accumulation')?.score || 0 },
    { day: 'Distribution', volume: temporal.phases?.find(p => p.phase === 'distribution')?.score || 0 },
  ] : [];
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6 animate-in slide-in-from-right-4 duration-500">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="ml-auto flex gap-3">
          <a 
            href={`https://dexscreener.com/search?q=${target}`} 
            target="_blank" 
            rel="noopener noreferrer"
            className="px-4 py-2 bg-slate-panel border border-slate-border rounded-lg text-xs font-bold hover:bg-white/5 transition-all flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-slate-panel/50 border border-slate-border rounded-2xl p-4">
          ...
          <div className="text-[10px] text-slate-500 font-mono mt-1">POOLED_RESERVES</div>
        </div>
        <div className="bg-slate-panel/50 border border-slate-border rounded-2xl p-4">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className={cn(
              ...
              AI-Powered Institutional Verdict
            </h3>
          </div>
          <p className="text-slate-400 leading-relaxed italic">
            {aiVerdict || 'Analyzing structural integrity and market coordination patterns...'}
          </p>
        </div>
